...
- (BOOL) removeExistingInstallerDirectory: (NSString *) path
{
    ...
    NSLog(@"Detected an existing installer directory at %@", path);
...
- (BOOL) attemptInstallerDirectoryCreation: (NSString *)path :(BOOL)removeExisting
{
    ...
    {
        if ([self removeExistingInstallerDirectory:path])
        {
            NSLog(@"Removed existing installer directory");
        }
    }

    NSLog(@"Attemping to create installer directory %@", path);
    ...
    {
        NSLog(@"Created installer directory at %@", path);
        return YES;
    }
    else
    {
...
-(NSString *) createInstallerDirectory:(NSString *)targetVolume
{
     ...
     * This installer plugin cannot pass information in memory to the pre/post install scripts of any of the packages in Puppet Enterprise
    ...
    pendingInstallerDir = [NSString stringWithFormat:@"%@%@", home, @"/Library/Caches/puppet-enterprise-installer"];
...
-(void) writeConfigFile:(NSString*)installerDir
{
    if (!(installerDir == (id)[NSNull null] || installerDir.length == 0))
    {
        ...
        NSURL *configFile = [[NSURL URLWithString:installerDir] URLByAppendingPathComponent:@"PuppetEnterpriseInterviewConfiguration.txt"];
